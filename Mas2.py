...
def create_star_catalog():
    ...
    data = {
        ...
        'RA_deg': [
            ...
            207.693, 116.594, 116.594, 116.594, 116.594, 116.594, 356.099, 356.099  # Approximate positions, moons close to planets
        ],
        'Dec_deg': [
            -18.4103, -18.4147, -18.3847, -18.4240, -18.3267, -18.5122, -18.3561, -18.3506, -18.2375, ATLAS_DEC + 10,
            -15.2113, 21.239, 21.239, 21.239, 21.239, 21.239, -4.1026, -4.1026
        ],
        'Magnitude': [
...
def auto_center_on_atlas(atlas_data, zoom, width, height):
    """Calculates the camera offset needed to center on the ATLAS star."""
    target_x_screen, target_y_screen = width / 2, height / 2
    
    raw_x, raw_y = world_to_screen_coords(atlas_data['RA_deg'], atlas_data['Dec_deg'], width, height, zoom, 0, 0)
...
while running and frame_count < max_frames:
    ...
    # --- QC ALGORITHM & ENTROPY LOGIC ---
    
    vivid_clarity_factor = run_vivid_clarity_factor()
    
    if quantum_sensor_power >= CORE_STABILITY_LOCK:
        planetary_instability *= 0.99 
        planetary_instability = max(0.01, planetary_instability)
    else:
        planetary_instability += random.uniform(-0.001, 0.005)
        planetary_instability = min(1.0, planetary_instability)
        
    calculated_entropy = planetary_instability * (1.0 - quantum_sensor_power)
    calculated_entropy = np.clip(calculated_entropy, 0.0, 1.0) 

    # BIOSIGNATURE METRIC: High Clarity + Low Entropy = High Biosignature Probability
    biosignature_metric = vivid_clarity_factor * (1.0 - calculated_entropy)
    biosignature_metric = np.clip(biosignature_metric * 1.5, 0.0, 1.0) 

    # ENTROPY-DRIVEN ZOOM CONTROL
    target_zoom = 1.0 + (MAX_ATLAS_ZOOM - 1.0) * calculated_entropy
    zoom_level = target_zoom
    
    # Stabilization for ATLAS View (Center Camera)
    camera_offset_x, camera_offset_y = auto_center_on_atlas(atlas_data, zoom_level, width, height)
    ...
    # 1. Draw the Quantum Shield/Earth (for context, even though we are zoomed in)
    draw_quantum_shield(quantum_sensor_power)

    # Draw the radio antenna (pulses during TX '1')
    draw_antenna(screen, current_bit, quantum_sensor_power)

    # 2. Draw the Star Field (Now zoomed and centered on ATLAS coordinates)
    draw_stars_and_threats(star_catalog, quantum_sensor_power, zoom_level, current_bit, camera_offset_x, camera_offset_y)
    
    # 3. Capture and Apply Realism Filters
    screen_copy = screen.copy() 
    filtered_surface = apply_realism_filters(screen_copy, calculated_entropy)
    screen.blit(filtered_surface, (0, 0)) 

    # 4. Draw ATLAS QML Close-Up Overlay
    atlas_close_up_surface = draw_atlas_close_up(atlas_surface, vivid_clarity_factor, calculated_entropy)
    screen.blit(atlas_close_up_surface, (ATLAS_X, ATLAS_Y))

    # NEW: Draw Sun Vivid Overlay (added as per request)
    solar_surface = draw_sun_vivid(solar_surface, vivid_clarity_factor, calculated_entropy, biosignature_metric)
    screen.blit(solar_surface, (SOLAR_X, SOLAR_Y))

    # 5. Draw Dashboard
    draw_dashboard(quantum_sensor_power, current_bit, current_char_index, calculated_entropy, biosignature_metric, vivid_clarity_factor)
