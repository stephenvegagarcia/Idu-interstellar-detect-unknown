#!/usr/bin/env python3
"""
Binary Transmitter visualization (fixed/cleaned version)

Fixes:
- draw_dashboard now takes current_bit_index instead of referencing an undefined global.
- Safe slicing/padding for binary characters at the end of the sequence.
- Bounded stream slice for display.
- Removed unused imports.
- Added keyboard controls: SPACE to pause/resume, P to force a sensor peak (for testing).
"""
...
def create_star_catalog(num_stars=100):
    ...
    # Insert sensor and a noise source for visualization
...
def draw_dashboard(sensor_power, current_bit, current_char_index, current_bit_index):
    ...
    # Calculate the full character from the current index (8 bits), safely (pad if needed)
    start = current_char_index * 8
    end = start + 8
    current_char_binary = NAME_BINARY_SEQUENCE[start:end].ljust(8, '0')  # pad if at end
    try:
        current_char_text = chr(int(current_char_binary, 2))
    except ValueError:
        current_char_text = '?'

    # Display a small window of the binary stream centered on the current bit (bounded)
    window_before = 8
    window_after = 8
    s = max(0, current_bit_index - window_before)
    e = min(BIT_COUNT, current_bit_index + window_after)
    stream_display = NAME_BINARY_SEQUENCE[s:e]
    ...
    bit_text = f"Bit [{current_bit_index}/{BIT_COUNT}]: {current_bit}"
    bit_render = font_small.render(bit_text, True, WHITE)
    panel_surface.blit(bit_render, (10, 60))

    panel_surface.blit(title_text, (10, 10))
    panel_surface.blit(status_line, (10, 140))  # moved down slightly to fit
...
logging.info("Starting Binary Transmitter simulation. Press SPACE to pause/resume, P to force peak, ESC or close window to quit.")
...
while running:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if event.key == pygame.K_SPACE:
                paused = not paused
                logging.info("Paused" if paused else "Resumed")
            elif event.key == pygame.K_p:
                # Force a manual peak for testing
                quantum_sensor_power = 1.0
                logging.info("Manual Q-SENSOR peak triggered (P pressed)")
    ...
    draw_stars_and_threats(star_catalog, quantum_sensor_power, zoom_level, NAME_BINARY_SEQUENCE[current_bit_index])

    # Pass the current bit and character index to the dashboard for display
    draw_dashboard(quantum_sensor_power, NAME_BINARY_SEQUENCE[current_bit_index], current_char_index, current_bit_index)
